...
\begin{table}[h]
\centering
\caption{Summary of processing-microstructure-property relationships in inorganic ferroelectrics, highlighting key rc achievements [68--70, 73, 74, 76].}
\label{tab:processing}
\begin{tabular}{p{3cm}p{4cm}p{4cm}p{4cm}}
\toprule
\textbf{Material / System} & \textbf{Key Processing Method(s)} & \textbf{Microstructural Outcome} & \textbf{Property Impact} \\
\midrule
BaTiO$_3$ ceramic (perovskite) & 
Controlled sintering (vary grain size) & 
1 \si{\micro\meter} grains $\rightarrow$ high domain wall density (optimal); finer grains $\rightarrow$ clamped domains & 
Max. permittivity at 1 \si{\micro\meter} grain (domain contribution peak); larger grains increase $d_{33}$ and $P_r$ if low defects. \\
\midrule
(K, Na)NbO$_3$-based ceramic (lead-free) & 
Multi-element doping (Sb, Bi, etc.) + tape-casting texturing & 
Tetragonal phase induced; nanoscale heterophase domains aligned with grain orientation & 
Ultrahigh piezoelectric response ($d_{33}$ \SI{800}{pC/N}) achieved; high $k_{33}$ and $T_c$ maintained. \\
\midrule
Hf$_{0.5}$Zr$_{0.5}$O$_2$ thin film (fluorite) & 
Interface engineering (electrode termination on LSMO); TiN substrate orientation control & 
Enhanced orthorhombic ferroelectric phase fraction; preferential domain orientation ((001)$_O$ axes out-of-plane) & 
Higher remanent $P_r$, stable ferroelectricity even at \SI{1.5}{nm} thickness, no wake-up cycling needed. \\
\midrule
Al$_{0.7}$Sc$_{0.3}$N thin film (wurtzite) & 
HiPIMS sputtering in N$_2$-rich plasma; epitaxial growth on 4H-SiC & 
Highly c-axis textured, low defect density (minimal N-vacancies, fewer grain boundary tilts) & 
Large polarization ($P_r > \SI{150}{\micro\coulomb\per\square\centi\meter}$) and fast switching; improved endurance due to fewer pinning sites (ongoing device tests). \\
\bottomrule
\end{tabular}
\end{table}
