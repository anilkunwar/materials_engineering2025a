...
\toprule
\textbf{Study} & \textbf{Approach} & \textbf{Key Contribution} \\
\midrule
Gu \textit{et al.} \cite{gu2014flexoelectricity} & Phase-field + DFT (domain wall structure) & Demonstrated that 180$^\circ$ domain walls in BaTiO$_3$ exhibit mixed Bloch–Néel character due to flexoelectric effects. Combined phase-field modeling with first-principles calculations, revealing intrinsic transverse polarization components at domain walls. \\
Chege \textit{et al.} \cite{chege2025strain} & DFT-calibrated phase-field (domain walls) & Calculated domain wall energies in PbTiO$_3$ and BaTiO$_3$ via DFT, providing parameters for the gradient energy coefficient in phase-field models. Ensured realistic wall energies and thicknesses, enhancing the predictive accuracy of domain structures. \\
Shao \textit{et al.} \cite{shao2025understanding} & Effective Hamiltonian + Phase-field (thermal hysteresis) & Combined first-principles-based effective Hamiltonian (atomistic Monte Carlo) with mesoscale phase-field simulations to investigate thermal hysteresis in BaTiO$_3$. Consistently captured the electrocaloric effect and first-order phase transitions across multiple scales. \\
Li \textit{et al.} \cite{liu2014theoretical} & DFT-informed Landau theory (sequential approach) & Derived Landau-Devonshire polynomial coefficients from first-principles calculations to parameterize phase-field simulations. Improved modeling accuracy of ferroelectric devices (e.g., thin-film capacitors) by using realistic DFT-based dielectric and piezoelectric constants. \\
\bottomrule
\end{tabular}
\end{table}

\subsection*{Data-Driven Enhancements in Ferroelectric Modeling}
\par In recent years, data-driven and machine learning (ML) techniques have been increasingly integrated into multiscale modeling workflows for ferroelectrics. These approaches primarily focus on two key applications: (1) the development of faster or more accurate surrogate models for atomic and mesoscale calculations using ML, and (2) the extraction and fitting of model parameters (e.g., free energy surfaces) from data assisted by ML.

\par A key advancement is the creation of machine-learned interatomic potentials that achieve accuracy similar to density functional theory (DFT). These potentials, including neural network and Gaussian approximation types, are trained on extensive DFT datasets. This training enables a precise representation of the potential energy surfaces of ferroelectric materials. This methodology has been exemplified by the training of a neural network potential for BaTiO₃ utilizing DFT data \cite{gigli2024modeling}. Large-scale molecular dynamics simulations (involving thousands of atoms) were performed using this surrogate potential to compute thermodynamic properties that would be impractical to obtain directly with density functional theory (DFT). Remarkably, first-principles accuracy was achieved for properties such as the Curie temperature, while finite-size artifacts common in small DFT supercells were circumvented \cite{gigli2024modeling}.

\par Machine learning has been utilized at the phase-field level to speed up simulations. The numerical solution of time-dependent Ginzburg–Landau and Poisson equations for complex three-dimensional domain structures is computationally intensive, often necessitating large-scale finite element or finite difference calculations. To mitigate this, machine learning (ML)-based surrogate phase-field models have been developed to learn microstructure evolution, enabling faster predictions. For instance, a neural network surrogate was introduced to emulate three-dimensional phase-field simulations of tip-induced switching in ferroelectric films. The surrogate model effectively adapted to the changing boundary conditions of the scanning probe microscopy (SPM) tip. It successfully predicted the pathways of domain switching while varying the tip positions and applied voltages. In comparison to traditional phase-field solvers, the machine learning model demonstrated speed improvements of up to 2500 times while keeping errors below 10\%. Such performance improvements facilitate real-time iterative design and inverse optimization of ferroelectric domain configurations—tasks that are computationally prohibitive with brute-force simulations. Various ML architectures have been explored for phase-field surrogates, including reduced-order models utilizing principal component analysis or autoencoders coupled with recurrent neural networks to predict microstructure evolution in a reduced feature space. Additionally, convolutional neural networks operating directly on three-dimensional microstructure grids have been implemented. Graph neural networks (GNNs) have also been applied to model the evolution of irregular grain structures and polycrystalline ferroelectrics, demonstrating success in predicting hysteresis loops for polycrystalline \ce{HfO2} with orders-of-magnitude speed improvements over traditional simulations. Representative data-driven contributions to ferroelectric modeling are summarized in Table~\ref{tab:ML_Surrogates} \cite{alhada2024machine}.

\par Another data-driven trend involves the use of machine learning (ML) for constructing free-energy landscapes. Instead of manual tuning of Landau polynomials to density functional theory (DFT) or experimental data, multi-dimensional free energy surfaces are now being fitted using ML algorithms. For example, in a recent study, a Landau-like free energy model for \ce{PbTiO3} was automatically generated by training an ML algorithm on a comprehensive dataset of equilibrium states derived from a second-principles effective Hamiltonian. This approach effectively captures the complex coupling between polarization, strain, and other order parameters, yielding a Landau potential that reproduces non-trivial behaviors—such as multiple phase transitions—without requiring \textit{a priori} assumptions. Moreover, ML ensures an optimal fit to the underlying data and can identify limitations in the chosen polynomial form (e.g., detecting the need for higher-order terms to avoid spurious minima) \cite{Pulzone2025}. As observed in prior research, the precision of thermodynamic potentials can be enhanced through data-driven free energy construction, which leverages comprehensive material datasets across varying temperature and field conditions, thereby mitigating bias inherent in manually selected functional forms \cite{liu2014theoretical}. This AI-assisted strategy, though still emerging, shows promise for systematically bridging first-principles calculations and continuum models, particularly in complex ferroelectrics where subtle couplings may elude human intuition.
\begin{table}[htbp]
\centering
\caption{Selected data-driven approaches enhancing ferroelectric modeling.}
\label{tab:ML_Surrogates}
\renewcommand{\arraystretch}{1.4}
\begin{tabular}{p{3cm} p{3.5cm} p{7.5cm}}
\toprule
\textbf{Study} & \textbf{Data-Driven Method} & \textbf{Contribution to Ferroelectric Modeling} \\
\midrule
Gigli \textit{et al.} \cite{gigli2024modeling} & ML interatomic potential (NN) & Trained neural network potential on DFT data for BaTiO$_3$, enabling large-scale atomistic simulations with DFT accuracy. Provided insights into how system size and functional choice influence critical ferroelectric properties such as Curie temperature. \\
Alhada-Lahbabi \textit{et al.} \cite{alhada2024machine} & Neural network surrogate for phase-field & Developed a surrogate neural network model for 3D phase-field simulations of domain switching under scanning probe microscopy (SPM) tips. Achieved $\sim2500\times$ speed-up with $<10\%$ error. Enabled rapid exploration and optimization of ferroelectric domain structures. \\
Pulzone \textit{et al.} \cite{Pulzone2025} & ML-fit Landau free-energy potential & Used supervised machine learning to fit a Landau-Devonshire free energy surface for PbTiO$_3$ from effective Hamiltonian data, accurately capturing subtle phase transitions and illustrating how thermal fluctuations affect the free-energy landscape. \\
Montes \textit{et al.} \cite{montes2021accelerating}; Peivaste \textit{et al.} \cite{peivaste2022machine}; Smith \textit{et al.} \cite{smith2024physics} & Physics-informed surrogates and reinforcement learning & Demonstrated real-time predictions of microstructure evolution and domain-wall manipulation using physics-informed machine learning models combined with reinforcement learning, optimizing probe-induced domain switching and polarization trajectories. \\
\bottomrule
\end{tabular}
\end{table}

\subsection*{Recent Progress and Multiscale Insights}
\par Recent advances in ferroelectric phenomena, previously considered intractable, have been achieved through the integration of density functional theory (DFT), phase-field modeling, and data-driven approaches. Key progress areas and successful applications of multiscale modeling are highlighted herein:
\begin{itemize}

\item \textbf{Thermal Phase Transitions and Hysteresis:} The origin of thermal hysteresis in BaTiO₃ was investigated through a bridged atomic and mesoscale modeling approach. An effective Hamiltonian, employed to capture microscopic fluctuations, was combined with phase-field simulations, which modeled macroscopic domain behavior. This multiscale framework revealed that the first-order ferroelectric phase transition generates a thermal hysteresis loop in polarization during heating and cooling. Notably, electrocaloric predictions from both scales aligned with experimental data, validating the methodology. The study established a concurrent multiscale simulation framework for ferroelectrics, enabling cross-validation between atomic-scale, continuum-scale, and experimental results. Additionally, it demonstrated how stabilizing field conditions can induce a gradual shift from first-order to second-order-like transition behavior—a critical insight for applications such as thermal actuators and temperature sensors \cite{shao2025understanding}.

\item \textbf{Domain Wall Structure and Dynamics:} Multiscale studies combining phase-field modeling (with flexoelectric coupling) and first-principles calculations reveal that 180\textdegree{} domain walls in BaTiO\textsubscript{3} exhibit a Bloch-N\'eel hybrid character, featuring in-plane polarization rotation driven solely by flexoelectricity. This previously overlooked effect influences wall conductivity and mobility. Quantum calculations resolve atomic-scale distortions, while continuum theory captures long-range interactions, enabling a complete mechanical description. First-principles-derived wall energies improve phase-field models, enhancing predictions of domain evolution in thin films like Pb(Zr,Ti)O\textsubscript{3} and HfO\textsubscript{2}-based systems, where precise energy values dictate size-dependent coercive fields \cite{gu2014flexoelectricity}.

\item \textbf{Defect and Grain-Boundary Effects in Ferroelectrics:} Material imperfections critically influence ferroelectric behavior, necessitating multiscale modeling. Point defects require atomic-scale treatment, while their collective effects on domains manifest at mesoscale. Phase-field models, parametrized using DFT or atomistic calculations, reveal defect impacts—such as oxygen vacancies—on hysteresis loops and coercive fields. Grain boundaries have been analyzed through large-scale phase-field simulations with first-principles-derived parameters, explaining polarization suppression in fine-grained or textured microstructures. Data-driven approaches (e.g., GNN surrogates) now enable high-throughput statistical studies of polycrystalline systems \cite{volker2011multiscale}.

\item \textbf{Negative Capacitance and Steep-Slope Electronics:} Ferroelectrics enable negative capacitance field-effect transistors (NC-FETs) by exploiting their unstable polarization regime ($P$--$E$ curve's negative slope), allowing voltage amplification when coupled with a dielectric. Multiscale modeling reveals that ferroelectric thin films stabilize negative capacitance without hysteresis via multi-domain configurations, where domain walls suppress depolarization fields, sustaining a quasi-static negative-$dP/dE$ regime. Phase-field simulations, incorporating electrostatic and elastic boundary conditions, validate this using experimentally or first-principles-derived free energy landscapes. First-principles calculations identify optimal ferroelectrics (e.g., Hf$_{0.5}$Zr$_{0.5}$O$_2$), while device models confirm domain wall motion---not uniform polarization---drives the effect. Performance depends critically on gradient energy coefficients and dielectric thickness, guiding NC-FET design for steep-slope switching \cite{saha2020multi}.

\item \textbf{Electrocaloric and Energy Harvesting Applications:} Ferroelectrics exhibiting large adiabatic temperature changes under applied fields (electrocaloric effect) or high energy conversion efficiency (for energy harvesting) are of significant interest for solid-state cooling and self-powered devices. Material discovery and optimization are facilitated by multiscale modeling approaches. For the electrocaloric effect, field-induced entropy changes over wide temperature ranges have been evaluated using phase-field modeling combined with effective Hamiltonian methods \cite{shao2025understanding}. Model calibration using first-principles-derived thermodynamic parameters allows quantitative prediction of electrocaloric strengths in ferroelectrics and guides the design of optimized multi-layer heterostructures. For energy harvesting (e.g., piezoelectric systems), simulations coupling ferroelectric domain dynamics with mechanical elasticity optimize domain texturing for maximum energy output. Data-driven methods efficiently explore parameter spaces (grain size, orientation distribution), surpassing traditional phenomenological approaches. Recent reviews highlight how phase-field and atomistic modeling accelerate lead-free piezoelectric development by clarifying composition and domain structure effects \cite{volker2011multiscale}.
\end{itemize}

\par In summary, significant advancements in understanding experimental observations and developing novel device concepts in ferroelectrics have been achieved through the integration of quantum-mechanical calculations, continuum phase-field modeling, and emerging AI tools. By bridging atomic-scale physics with macroscopic behavior, researchers can now more confidently address key questions, such as the atomic-level origins of device-scale phenomena or the influence of chemical composition on domain-switching dynamics.

\subsection*{Challenges in Bridging Scales}
\par In spite of the significant advancements, several challenges persist in attaining a seamless multiscale understanding of ferroelectricity:
\begin{itemize}

\item \textbf{Parameter Consistency and Transfer:} Reliable material parameters that remain valid across scales are challenging to obtain \cite{volker2011multiscale}. Landau coefficients derived from first-principles calculations at 0 K often require adjustment to account for thermal effects at finite operating temperatures. In some cases, direct fitting to experimental data remains necessary—for instance, to calibrate Landau coefficients such that phase-field models accurately reproduce measured hysteresis loops or dielectric responses. Ensuring consistency between density functional theory (DFT), intermediate models, and phase-field parameters is labor-intensive. While automated fitting methods, including machine learning-based approaches, show promise, they must maintain physical consistency and avoid overfitting to unphysical artifacts \cite{saha2020multi}.

\item \textbf {Bridging Temporal Scales in Ferroelectric Switching:} Ferroelectric switching spans picosecond atomic vibrations to years-long fatigue. Fast processes (e.g., phonon dynamics) are modeled with DFT and molecular dynamics, while slower phenomena (e.g., domain wall motion) are described by phase-field methods. Bridging these scales requires coarse-grained kinetics (e.g., kinetic Monte Carlo) to map atomistic probabilities to continuum parameters. However, accuracy is challenging under defects or high-frequency fields. Advanced methods like accelerated molecular dynamics aid rate estimation, but integration with phase-field models remains complex.

\item \textbf {Scale of Microstructure vs. DFT Cell:} Ferroelectric phenomena involving long-range interactions (e.g., strain fields, depolarization fields) require large simulation domains. Phase-field methods model micron-scale domains, while DFT is limited to nanometers, preventing direct observation of features like strain accommodation and domain branching. Intermediate methods (e.g., effective Hamiltonian Monte Carlo) extend simulations to 10–20 nm but remain below continuum scales. Near the Curie point, fluctuations reduce mean-field accuracy, though dipole-dipole interactions suppress them in most cases. For critical regions or disorder effects (e.g., relaxors), fluctuations must be captured via large supercells or phase-field fluctuations (e.g., Langevin noise), both computationally intensive \cite{liu2014theoretica}.

\item \textbf{Complex Chemistries and Phases:} Recent ferroelectric research has expanded beyond traditional perovskites (e.g., BaTiO$_3$, PZT) to include hafnia-based thin films, hybrid organic--inorganic perovskites, and two-dimensional systems, where competing order parameters (e.g., antiferroelectric/ferroelectric coexistence) and complex local chemistries (dopants, solid solutions) present modeling challenges. The development of multiscale models is hindered by high-dimensional Landau free energy landscapes and computationally demanding density functional theory (DFT) calculations, particularly for disordered or low-symmetry structures---for instance, stabilized ferroelectric HfO$_2$ requires dopants or strain, resulting in mixed phases that resist simple energy expansions. Current research focuses on incorporating phase coexistence into phase-field models through first-principles parameter extraction and data-driven approaches for high-dimensional fitting, though enforcing fundamental physical constraints (e.g., stability conditions, symmetry requirements) remains an ongoing challenge.

\item \textbf{Validation and Uncertainty:} Increased reliance on modeling necessitates rigorous validation. Experimental validation of multiscale predictions remains challenging—for instance, intrinsic Landau coefficients and domain wall energies are often measured indirectly. Uncertainty propagation occurs in density functional theory (DFT), where uncertainties from functional choice affect phase-field outputs. Quantifying these uncertainties is essential for model-based design, particularly in industrial applications. Potential solutions, such as Bayesian calibration or uncertainty quantification (e.g., via ensembles of ML surrogate models), are still in early stages for ferroelectric systems.

\item \textbf{Computational Load:} Comprehensive multiscale simulations remain computationally demanding despite data-driven accelerations. Large 3D phase-field simulations with nanoscale resolution and extensive parameter spaces—particularly when conducting statistical analyses or optimizations—can strain modern supercomputers. While machine learning (ML) surrogates reduce runtime costs, they incur significant offline training expenses and necessitate large training datasets, often derived from simulations. Efficient integration of multiscale components (e.g., DFT → MD/ML → phase-field → device models) into a unified workflow presents an engineering challenge. However, advancements like GPU-accelerated solvers (e.g., FerroX) and open-source multiphysics frameworks are enabling these studies, while improvements in hardware (e.g., GPUs, TPUs) further reduce computational burdens.
\end{itemize}

\par In light of these challenges, Hybrid approaches—such as adaptively embedding fine-scale regions in coarse models or using physics-informed machine learning to enforce physical laws—are being developed to address these challenges. Standardized multiscale workflows for ferroelectrics are also prioritized, including databases of first-principles-derived coefficients for phase-field models, ensuring reproducibility and transferability.

\subsection*{Conclusions}
\par Multiscale computational modeling---integrating density functional theory (DFT), phase-field methods, and data-driven techniques---has emerged as a transformative approach in ferroelectric research. DFT provides fundamental quantum-scale insights into atomic distortions and domain wall structures, while phase-field simulations effectively bridge these to mesoscale phenomena, incorporating critical effects of defects interfaces, and device geometries. The incorporation of machine learning has further accelerated progress, enabling real-time device simulations and automated parameter optimization. Future developments are expected to focus on three key areas: (1) enhanced DFT functionals and \textit{ab initio} methods for improved accuracy, (2) advanced phase-field formulations incorporating additional physics (e.g., multiferroic coupling and microstructural evolution), and (3) tighter multiscale integration through machine learning techniques such as active learning and neural PDE solvers. This integrated framework has already yielded significant advances in understanding conventional ferroelectrics (\ce{BaTiO3}, \ce{PbTiO3}) and developing next-generation materials (HfO$_2$-based thin films, lead-free piezoelectrics). As scale-bridging challenges are addressed, these methods will increasingly transition from explanatory tools to predictive platforms for discovering novel ferroelectric materials and engineered domain architectures.
